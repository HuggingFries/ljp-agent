#!/usr/bin/env python3
"""
RAG Agent for LJP with negative-example retrieval (error-reason based).

Workflow:
1. Extract legal elements from input fact
2. Retrieve similar historical cases from unified KB
3. Inject case info (error analysis only) into prompt
...
- 每个案例包含案件关键事实概括和该案例曾犯过的错误分析。
- 历史案例中的"易错点"指出了同类案件中常见的错误（如罪名认定偏差、刑期高估/低估、情节认定错误等）。
- 请你**参考这些易错点，避免犯同样的错误**，结合当前案件事实独立判断。
...
class LJPRAGAgent:
    """
    Main RAG Agent for Legal Judgment Prediction with error-based historical cases.
    ...
    def __init__(
        ...
        logger.info(f"LJPRAGAgent initialized successfully")
    ...
    def format_retrieved_cases(
        ...
        Only includes case_summary and error_reason (易错点).
        """
